...
| 67 | Focused UI | #60 | Sticky centered five-workspace top navigation. | Contract + browser | `tests/workspace-navigation-page.test.mjs:59`; `tests/browser/browser-helpers.mjs:329` | Today, Data, Recommendations, Orders, Analysis are shared and all activate. | LOCAL-COVERED |
...
| 69 | Focused UI | #60 | Default to Today with readiness/risk/next action. | Unit + browser | `tests/workspace-navigation.test.mjs:21`; `tests/browser/public-smoke.spec.mjs:12` | Invalid/empty state resolves to Today with summary and one action. | LOCAL-COVERED |
...
| 74 | Orders UI | #60/#58 | Use one three-group segmented control. | Contract + browser | `tests/workspace-navigation-page.test.mjs:131`; `tests/browser/browser-helpers.mjs:404` | One radiogroup exposes Taiwan, Vietnam, and Subcontract; only selected table is shown. | LOCAL-COVERED |
