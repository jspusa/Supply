...
Operator-maintenance portion superseded by ADR 0004. ADR 0007 supersedes the schema v2 date-window rule below for schema v3: released Packaging Specification Versions remain immutable, their historical date windows may overlap, and `newOrderPackagingDefaultVersion` explicitly selects the version for new work. The canonical schema and per-site fallback design remain in force.
...
Under the historical schema v2 contract, packaging changes for one Product SKU were retained as dated versions with exactly one open-ended current version and overlapping date windows were invalid. Schema v3 instead follows ADR 0007's explicit new-work default while retaining immutable history. Country of origin and standard factory are separate facts, so an unknown origin remains `null` instead of being inferred from Supply's TW/VN order grouping, and a `7`-prefixed Order SKU is always routed as a Subcontract Order.
